...
class DbManager():
    ...
    def remove_ride_no_driver_found(self, username):
        ...
        cursor.execute("select ride_id from rides where username=\""+username+"\";")
    ...
    def active_users(self,username,password):
        ...
        insert_query = "insert into active_users values(" + "\"" + user_data[0] + "\"," + "\"" + user_data[
            1] + "\"," + "\"" + user_data[2] + "\"," + "\"" + \
                       str(user_data[3]) + "\"," + "\"" + user_data[4] + "\"," + "\"" + user_data[5] + "\"," + "\"" + user_data[6] + "\""+ ");"
        cursor.execute(insert_query)
        conn.commit()
        cursor.close()
        conn.close()
    ...
    def submitted_rides(self,ride_data):
        ...
        insert_query = "insert into rides values(" + "\"" + ride_data[0] + "\"," + "\"" + ride_data[
            1] + "\"," + "\"" + ride_data[2] + "\"," + "\"" + \
                       ride_data[3] + "\"," + "\"" + ride_data[4] + "\"," + "\"" + ride_data[5] + "\"," + "\"" + ride_data[6] + "\",\"PENDING\",\"\");"

        insert_query_to_distribution_table = "insert into ride_distribution values(\"" +ride_data[6]+"PENDING\"," + "\"" + ride_data[6] + "\"," + "\"PENDING\",\"\");"
        cursor.execute(insert_query)
    ...
    def modify_ride_distribution(self,ride_id,acceptance_result,drivername):
        ...
        if (acceptance_result=="reject"):
            insert_query_to_distribution_table = "insert into ride_distribution values(\""+ride_id + "REJECT-"+drivername+ "\","  + "\"" + ride_id + "\"," + "\"REJECT-"+drivername+"\",\""+drivername+"\");"
            cursor.execute(insert_query_to_distribution_table)

        else:
            query = "UPDATE ride_distribution SET ride_status =\"ACCEPT\",driver_name=\""+drivername+"\" " + "WHERE ride_id=\"" + ride_id + "\" and ride_status_name=\""+ride_id+"PENDING"+"\";"
            query1 = "UPDATE rides SET ride_status =\"ACCEPT\",driver_name=\""+drivername+"\" "  + "WHERE ride_id=\"" + ride_id + "\";"
            cursor.execute(query1)
    ...
    def fetch_rides_distribution_of_driver(self,drivername):
        cursor, conn = self.connect_to_db()
        query = "select * from ride_distribution where driver_name=\"" + drivername + "\";"
        cursor.execute(query)
    ...
    def fetch_ride_details_customer_history(self,username):
        cursor, conn = self.connect_to_db()
        query = "select * from rides where username=\"" + username + "\"and ride_status in (\"COMPLETED\",\"VERIFICATION-FAILED\");"
        cursor.execute(query)
